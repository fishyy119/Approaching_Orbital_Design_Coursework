...
- 对于远程转移，机动脉冲定义为出发脉冲与到达脉冲：
  - `Delta v1 = v1 - v0`
  - `Delta v2 = vf - v2`
- 若需比较脉冲大小，统一使用 `norm(Delta v)`；也就是先做速度矢量差，再取模，不用标量速度差或速度模长之差替代。
...
- 先从惯性系下追踪器与目标的绝对运动出发。
- 定义相对位置向量 `xi = rc - rt`。
- 为避免与后续标量趋近律 `rho(t)` 混淆，论文写作中优先用 `xi` 表示 LVLH / CW 相对位置向量；若代码局部仍使用 `rho` 命名，则按该文件上下文理解。
...
- 理论表述优先采用三维 LVLH/CW 形式；二维共面模型是令 `z = zdot = az = 0` 后得到的平面特例。
- 本仓库使用的标准微分方程对应为：
...
- 直线脉冲路径
  - 用目标反方向单位矢量定义空间路径方向。
- 中间点按 `r_m = r_T + rho(t_m) * rho_hat` 形式生成，其中 `t_m = mT / N`，`m = 1,2,...,N-1`。
- 抛物线脉冲路径
  - 用路径几何约束替代直线方向。
...
- 总速度增量等于各段脉冲向量模长与末端速度修正向量模长之和。
- 实现时统一写成 `sum_k norm(v_k^+ - v_k^-) + norm(v_f - v_N^-)`，不以标量速度差代替。
...
- 圆轨道目标的 CW 平面自然绕飞可写为 `x = k sin(theta)`、`y = 2 k cos(theta) + y_c`、`theta = n t + theta_0`，平面投影是半轴比为 `1:2` 的椭圆；共面算例对应法向幅值为零。
- 遍历巡游轨道
...
- 若需要与 STK 的局部机动坐标对照，远程导引段的脉冲矢量可在机动时刻投影到局部 VNC 系；当前二维共面实现仅输出面内二维 `[V, C]^T` 分量，其中 `V` 沿瞬时速度方向，`C` 由 `V x N` 完成右手系，法向 `N` 分量恒为零。
...
- `CW_Transfer/cw_stm.m` 仍作为课程仓库中的三维规范实现保留；论文理论部分应优先写三维 `3x3` 分块矩阵。近距离导引目录下另有一份本地 `cw_stm_2d.m`，直接输出二维共面 `2x2` 分块，供近距离导引脚本调用。
