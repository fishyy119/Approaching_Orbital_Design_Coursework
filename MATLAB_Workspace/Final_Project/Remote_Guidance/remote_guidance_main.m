...
%% 轨迹与动画参数
trajectory_wait_step = 30; % s
trajectory_transfer_step = 20; % s
reference_orbit_samples = 720;
animation_frame_count = 240;
animation_pause_seconds = 0.02;
animation_gif_delay = 0.04;
animation_save_gif = true;
animation_gif_name = 'remote_guidance_transfer.gif';
%% 任务场景参数
target = struct();
...
for i_td = 1:num_td
    ...
    for i_dt = 1:num_dt
        ...
        for i_branch = 1:numel(branch_names)
            ...
            rho_dot_minus = capture_info.C_I2L * (v2_tr - v_ta) - capture_info.n * S * rho_cap_L;
...
%% 自适应等高线图显示数据
contour_dv_upper_limit = 1000; % m/s，等高线显示用绝对 Δv 上限
contour_level_count = 10; % 绘制的等高线数量
contour_width = 1; % 等高线粗
colorbar_tick_count_max = 6; % 颜色条最多显示的主刻度数

contour_plot_config = struct();
contour_plot_config.dv_upper_limit = contour_dv_upper_limit;
contour_plot_config.level_count = contour_level_count;
contour_plot_config.contour_width = contour_width;
contour_plot_config.colorbar_tick_count_max = colorbar_tick_count_max;

contour_plot_info = plot_remote_guidance_window_contours( ...
...
if isfinite(best.dv_total)
    ...
    simulation_config.wait_step = trajectory_wait_step;
    simulation_config.transfer_step = trajectory_transfer_step;
    simulation_config.reference_orbit_samples = reference_orbit_samples;
    ...
    plot_remote_guidance_trajectory_2d(simulation_result);

    animation_config = struct();
    animation_config.frame_count = animation_frame_count;
    animation_config.pause_seconds = animation_pause_seconds;
    animation_config.gif_delay = animation_gif_delay;
    animation_config.save_gif = animation_save_gif;
    animation_config.gif_path = fullfile(output_dir, animation_gif_name);

    animation_info = render_remote_guidance_animation_2d(simulation_result, animation_config);
    remote_result.animation = animation_info;
else
    warning('未找到最优 Lambert 窗口，故不绘制轨迹与动画。');
end
